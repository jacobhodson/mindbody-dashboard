 ...
 *   birthdays     – sorted by days_until asc
 *   anniversaries – sorted by days_until asc
 */
...
export const handler = async (event) => {
  ...
  try {
    ...
    // Fetch all active clients — we need BirthDate + CreationDate
    let allClients = [];
    let offset = 0;
    while (true) {
        ...
        if (!isNaN(bd.getTime()) && bd.getFullYear() > 1900) {
          ...
          if (days <= WINDOW_DAYS) {
            ...
            const nextYear = (new Date(now.getFullYear(), month - 1, day) <= now)
              ? now.getFullYear() + 1
              : now.getFullYear();
            const age  = nextYear - bd.getFullYear();
            birthdays.push({
              id:       String(c.Id),
              name,
              email:    c.Email || '',
      ...
      if (c.CreationDate) {
        ...
        if (!isNaN(cd.getTime())) {
          ...
          if (days <= WINDOW_DAYS) {
            ...
            if (years < 1) continue; // skip if less than 1 year (first year not yet complete)
            anniversaries.push({
              id:       String(c.Id),
              name,
              email:    c.Email || '',
    ...
    return ok({ birthdays, anniversaries });
  } catch (e) {
