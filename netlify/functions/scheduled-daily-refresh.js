...
// would), but it still never auto-fired even once in 5 days. Whatever the
// cause (a Netlify plan/account gate on actual execution is the leading
// theory), this is now triggered by Supabase pg_cron + pg_net instead —
// see 20260921000000_scheduled_job_cron.sql — which calls this function's
// plain HTTP endpoint on a schedule and can actually be verified to fire.
// Left as a plain function (no netlify.toml schedule) so that call reaches
// it; this config.schedule export is now just documentation of intent.
export const config = {
