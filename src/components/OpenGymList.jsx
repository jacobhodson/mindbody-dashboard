    ...
    <div className="rounded-xl border border-gray-200 bg-white flex flex-col">
      <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">Open Gym</h2>
          {!loading && (
            <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-xs font-medium text-blue-600 border border-blue-500/20">
              {clients.length}
            </span>
          )}
        </div>
      ...
      <div className="overflow-y-auto max-h-64 scrollbar-thin">
        ...
        {!loading && !error && clients.length === 0 && (
          <div className="py-8 text-center">
            <CheckCircle className="h-7 w-7 text-gray-300 mx-auto mb-2" />
            <p className="text-sm text-gray-400">No open gym sessions this week</p>
          </div>
        )}
        ...
        {!loading && !error && clients.map((client) => (
          <div
            key={client.id}
            className="flex items-center justify-between px-5 py-2.5 border-b border-gray-200/50 last:border-0 hover:bg-gray-200/30 transition-colors"
          >
            ...
            <span className="shrink-0 rounded-full border border-blue-500/20 bg-blue-500/10 px-2.5 py-0.5 text-xs font-semibold tabular-nums text-blue-600">
