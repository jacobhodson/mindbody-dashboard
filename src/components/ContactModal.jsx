  ...
  return (
    <div className="flex gap-3 text-xs">
      <div className="shrink-0 mt-0.5">
        <div className="h-2 w-2 rounded-full bg-emerald-500/60 ring-2 ring-white mt-1" />
      </div>
      <div className="min-w-0">
        <p className="text-gray-600">
          {formatDistanceToNow(date, { addSuffix: true })}
          <span className="ml-2 text-gray-400">{format(date, 'd MMM yyyy, h:mm a')}</span>
        </p>
        {entry.note && (
          <p className="text-gray-700 mt-0.5 italic">"{entry.note}"</p>
        )}
      </div>
    </div>
  );
...
export default function ContactModal({ client, onClose, onContacted, logContact, getClientLogs }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-gray-300 bg-white shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-0.5">Contact client</h3>
        <p className="text-sm text-gray-600 mb-5">{client.name || 'Unknown client'}</p>
        ...
        {/* Contact details */}
        <div className="rounded-lg bg-gray-200 px-4 py-3 mb-5 space-y-1.5 text-sm">
          {client.email && (
        ...
        <div className="mb-5">
          ...
          {history !== null && history.length > 0 && (
            <div className="space-y-3 border-l border-gray-300 pl-3">
        ...
        <label className="block text-xs text-gray-600 mb-1.5">
          Note <span className="text-gray-400">(saved to contact log)</span>
        </label>
        <textarea
          value={note}
          ...
          className="w-full rounded-lg border border-gray-300 bg-gray-200 px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-emerald-500 focus:outline-none resize-none mb-4"
        />

        <button
          onClick={handleMark}
          disabled={status === 'loading' || status === 'done'}
          className="w-full flex items-center justify-center gap-2 rounded-lg bg-gray-300 hover:bg-gray-400 disabled:opacity-60 px-4 py-2.5 text-sm font-medium text-gray-900 transition-colors"
        >
        ...
        {status === 'error' && (
          <p className="mt-2 text-xs text-red-600 text-center">Something went wrong — try again.</p>
        )}
      </div>
    </div>
  );
