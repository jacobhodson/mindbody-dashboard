...
import {
  ...
  MessageSquare, ClipboardList, Trash2, Dumbbell,
} from 'lucide-react';
...
export default function ClientDetail({ mindbodyId, isManager, staff, onBack }) {
  const {
    client, visits, notes, contactLogs, linkedTasks, loading, error,
    updateCaseload, updateNextProgramDue, addNote, deleteNote,
          ...
          <div>
            ...
            {isManager ? (
              <select
                ...
                onChange={(e) => updateCaseload(e.target.value || null)}
              ...
              <p className="text-sm text-gray-800">{staffNameById[client.assigned_staff_id] || 'Unassigned'}</p>
            )}
          </div>
          <div>
