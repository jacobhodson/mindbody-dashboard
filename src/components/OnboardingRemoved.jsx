...
export default function OnboardingRemoved({ clients = [], setDecision }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white/60 flex flex-col">
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <span className="rounded-full bg-gray-200 px-2 py-0.5 text-xs font-medium text-gray-500 border border-gray-300">
          {clients.length}
        </span>
        <ChevronDown className={`ml-auto h-3.5 w-3.5 text-gray-400 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="divide-y divide-gray-200/60 border-t border-gray-200">
          {clients.map((client) => (
            <div key={client.id} className="flex items-center gap-3 px-5 py-2.5">
              <div className="min-w-0 flex-1">
                <p className="text-sm text-gray-600 truncate">{client.name || 'Unknown'}</p>
              ...
              <button
                ...
                className="shrink-0 flex items-center gap-1 rounded-lg border border-gray-300 bg-gray-200 px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-300 hover:text-gray-800 transition-colors"
              >
