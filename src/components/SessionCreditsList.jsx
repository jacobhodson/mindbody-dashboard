...
function creditColor(n) {
  if (n >= 10) return 'text-red-600 bg-red-500/10 border-red-500/20';
  if (n >= 5)  return 'text-amber-600 bg-amber-500/10 border-amber-500/20';
    ...
    <div className="rounded-xl border border-gray-200 bg-white flex flex-col">
      <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">Accumulated Credits</h2>
          {!loading && clients.length > 0 && (
            <span className="rounded-full bg-yellow-500/10 px-2 py-0.5 text-xs font-medium text-yellow-600 border border-yellow-500/20">
              {clients.length}
            </span>
          )}
        </div>
      ...
      <div className="overflow-y-auto max-h-80 scrollbar-thin">
        ...
        {!loading && !error && clients.length === 0 && (
          <div className="py-8 text-center">
            <p className="text-sm text-gray-400">No accumulated credits detected</p>
            <p className="text-xs text-gray-300 mt-1">Only clients seen in the last 28 days are checked</p>
          </div>
        )}
        ...
        {!loading && !error && clients.map((client) => {
          ...
          return (
            <div key={client.id} className="border-b border-gray-200/50 last:border-0">
              <div
                className="flex items-center justify-between px-5 py-3 hover:bg-gray-200/30 transition-colors cursor-pointer select-none"
              ...
              {isExpanded && client.services?.length > 0 && (
                <div className="px-5 pb-3 pt-1 bg-gray-200/20 border-t border-gray-200/50 space-y-1.5">
                  {client.services.map((svc, i) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      <span className="text-gray-600 truncate">{svc.name || 'Unnamed service'}</span>
                      <span className="shrink-0 ml-3 text-gray-700 font-medium tabular-nums">
      ...
      {!loading && clients.length > 0 && (
        <div className="px-5 py-3 border-t border-gray-200">
          <p className="text-xs text-gray-400">Checked for clients active in last 28 days · up to 60 clients</p>
        </div>
      )}
    </div>
  );
